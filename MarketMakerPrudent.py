...
# 板情報からある一定の閾値を越えるAsk/Bid(=指値を入れる基準値)の位置を計算する関数
...
while True:
    ...
    if pos == 'none' or pos == 'entry':
        ...
        try:
            ...
            if int((ask - 1500)) < int(ticker["last"]):
                trendthres = "buy"
                if int((bid + 1500)) > int(ticker["last"]):
                    trendthres = "stay"
            elif int((bid + 1500)) > int(ticker["last"]):
                trendthres = "sell"
                if int((ask - 1500)) < int(ticker["last"]):
                    trendthres = "stay"
            else:
        ...
        try:
            ...
            if spread > SPREAD_ENTRY:
                ...
                logger.info('trendthres:%s ', trendthres);
